'use strict';
import { prompt as _prompt, Separator } from "inquirer";
import { generateMnemonic, generateSeed, generateBase58, generateAddress } from "./cli.js";
...
let secretPrompt = () => {
    let prompt = [{
    ...
    },
        {
            type: "list",
            name: "language",
            message: "注记词的语言(mnemonic language)",
            choices: [
...
let hdQuestion = (answers) => {
    let prompt = [
        ...
        },
        {
            type: "input",
            name: "indexRange",
            message: "输入账户生成索引(index) (e.g 1-100 or 1)",
            validate: function (val) {
                let reg = new RegExp("^[0-9]*$");
                if (reg.test(val)) { // 校验位数
                    return true;
    ...
    return _prompt(prompt).then(res => {
